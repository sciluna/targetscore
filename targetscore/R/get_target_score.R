...
get_target_score <- function(wk, wks, dist_ind, edgelist, n_dose, n_prot, proteomic_responses,
                             n_perm, verbose = TRUE, ts_pathway_scale = 1, fs_dat, neighbor_directions = c("upstream"), p_value_calculation = "together",
  ...
  for (neighbor_direction in neighbor_directions){
    ...
    results <- list(wk=wk, wks=wks, 
                    ...
                    debug_ts=debug_ts)
    results_all_directions[[neighbor_direction]]<-results
  }
  return(results_all_directions)
}
